...
export async function GET(
  ...
  try {
    ...
    const filter = url.searchParams.get('filter') || 'all'; // all, found, missing
    const includeSurplus = url.searchParams.get('surplus') === 'true';
    const includeNotes = url.searchParams.get('notes') === 'true';
    ...
    // Apply filters
    if (filter === 'found') {
      flatItems = flatItems.filter((i: any) => (i.quantityFound ?? 0) > 0);
